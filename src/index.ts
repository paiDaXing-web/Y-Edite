...
export default class MiniSandbox {
  ...
  constructor(options = {} as OptionsType) {
    ...
    // 设置防抖
    this.run = debounce(this.render, this.defaultConfig.autoRunInterval).bind(
      this
    );
    // 初始化
    this.init().then(() => {
  ...
  initOptions(options: OptionsType) {
    ...
    this.fileList = Object.keys(options.files || {}).map((filename) => {
      ...
      const _file = {
        ...
        ...file,
        filename:
          filename.lastIndexOf(".") > -1 ? filename : filename + ".html",
  ...
  private initDom() {
        ...
        <div class="sandbox-tab">
          ${this.fileList
            ...
            .map((file, index) => {
              const className =
                "sandbox-tab-item" +
                (this.fileIndex === index ? " sandbox-tab-active" : "");
              return `
    ...
    tabBar.addEventListener("click", (e) => {
      const targetEl = (e["path"] as HTMLElement[]).find(
        (dom) => dom.className === "sandbox-tab-item"
      );
      if (targetEl) {
  ...
  public async getResource(src: string): Promise<string> {
    ...
    const ldqResource: { [key: string]: string | Promise<string> } =
      window["ldqResource"];
  ...
  public triggleDirection(direction: DefaultConfigType["direction"] = "row") {
    ...
    const gutter =
      ["100%", "0%"].indexOf(defaultConfig.editorRange) > -1 ? "0px" : "5px";
  ...
  public async render(isReload = false) {
    ...
    for (const loader of fileLoaders) {
      template = await Promise.resolve(
        loader.call(this, template, currTemplate)
      );
    }
    iframeDocument.open();
